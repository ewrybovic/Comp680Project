...
class LabelReader:
    ...
    def process_text(self, text: str) -> dict:
        ...
        items_to_find = [
            ...
            "protein",
            "protoin",
        ]  # PyTesseract gets this confused sometimes
...
if __name__ == "__main__":
    ...
    reader = LabelReader(path_to_superres_model="ESPCN_x3.pb", is_Windows=False)

    image_path = str(Path.cwd() / "test_image" / "cropped_image.jpg")
    label_img = cv2.imread(image_path)

    text = reader.read_label(
        label_img, upres=True, sharpen=True, threshold=True, debug=True
    )
    print(text)
