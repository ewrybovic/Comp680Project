...
label_reader = LabelReader(
    path_to_superres_model=r"../LabelReader/ESPCN_x3.pb", is_Windows=False
)
...
def analyze_image(request):
    if request.method == "POST" and request.FILES.get("image"):
        ...
        if len(cropped_image) > 0:
            label_data = label_reader.read_label(
                cropped_image, threshold=False, debug=True
            )
...
def nutrition_summary(request):
    food_list = Serving.objects.all()
    return render(request, "Frontend/food_list.html", {"food_list": food_list})
